...
function ClaimCard({
  ...
  const accentBorder = item.erhalten ? C.settled : C.positive;

  return (
    <Animated.View
      style={[
        layout.card,
        item.erhalten && layout.cardSettled,
    ...
    >
      <View style={layout.cardTop}>
        <View style={layout.cardInfo}>
          <Text style={[layout.cardPerson, { color: item.erhalten ? C.textDim : C.textPrimary }]}>
            {item.person}
          </Text>
          {item.beschreibung ? (
        ...
        <View style={layout.cardMeta}>
          <Text style={[layout.cardAmount, { color: item.erhalten ? C.textDim : C.positive }]}>
            +{fmt(item.betrag)}
          </Text>
...
export default function ForderungenScreen() {
  ...
  const openTotal = forderungen.filter(f => !f.erhalten).reduce((sum, f) => sum + f.betrag, 0);
    ...
    <View style={[layout.safe, { backgroundColor: C.bg }]}>
      ...
      >
        ...
        {forderungen.length === 0 ? (
          ...
          <>
            {open.map((f, i) => (
              <ClaimCard key={f.id} item={f} index={i} onToggle={() => toggleErhalten(f.id)} />
            ))}
            {received.length > 0 && open.length > 0 && (
              <Text style={[layout.dividerLabel, { color: C.textDim }]}>ERHALTEN</Text>
            )}
            {received.map((f, i) => (
              <ClaimCard key={f.id} item={f} index={open.length + i} onToggle={() => toggleErhalten(f.id)} />
            ))}
          </>
        )}
      </ScrollView>
